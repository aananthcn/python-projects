...
squash = sigmoid # an 'S' shaped curve starts at (-x, 0), through (0, 0.5) and ends at (x, 1)
...
class ANN:
    ...
    def forward_propagate(self):
        """
        Propagate the input signal forward through the network. Refer
        https://en.wikipedia.org/wiki/Feedforward_neural_network for more
        details on the algorithm used.
        """
        # Exclude the last layer (as it is the output layer?)
    ...
    def get_output(self):
        output_layer = self.layers[-1] # last layer
...
class Layer:
    def __init__(self, id, layer_size, prev_layer_size):
        self.id = id
        self.n_neurons = layer_size  # = to number of input variables
        self.bias_val = 1
...
if __name__ == '__main__':
    ...
    print("Predictions without any training:")
